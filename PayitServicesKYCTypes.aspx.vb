...
Partial Class PayitServicesKYCTypes
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            Dim kyctype = (From c In db.KYCTypes Order By c.ID Descending Select New With {Key .Name = c.Name, Key .ID = c.ID}).ToList()
            ddlKYCType.DataSource = kyctype
            ddlKYCType.DataTextField = "Name"
            ddlKYCType.DataValueField = "ID"
            ddlKYCType.DataBind()
            ddlKYCType.Items.Insert(0, New ListItem("Select", "None"))

            ddlKYCTypeEdit.DataSource = kyctype
            ddlKYCTypeEdit.DataTextField = "Name"
            ddlKYCTypeEdit.DataValueField = "ID"
            ddlKYCTypeEdit.DataBind()
            ddlKYCTypeEdit.Items.Insert(0, New ListItem("Select", "None"))
    ...
    Protected Sub btnSubmit_Click(sender As Object, e As EventArgs) Handles btnSubmit.Click
        ...
        If Page.IsValid Then
            ...
            If (ddlKYCType.SelectedIndex = 0 Or ddlService.SelectedIndex = 0 Or txtRating.Text = "") Then
            ...
            Else
                ...
                Dim sql1 As String = "SELECT * FROM [payit].[dbo].[PayitServicesKYCTypes] WHERE [KYCTypeID]=@typeID and [PayitServiceID]=@serviceID and [KYCRating]=@rating"
                Dim cmd1 As SqlCommand = New SqlCommand(sql1, cn)
                cmd1.Connection = cn
                cn.Open()
                cmd1.Parameters.AddWithValue("@typeID", ddlKYCType.SelectedValue)
                ...
                Using reader As SqlDataReader = cmd1.ExecuteReader()
                    If reader.HasRows Then
                        ...
                        alertme("Type Already exists for:<br><b> " & ddlKYCType.SelectedItem.Text & "</b>")
                    Else
                        ' Record does not exist, add them
                        Dim cmd As SqlCommand = New SqlCommand("INSERT INTO [payit].[dbo].[PayitServicesKYCTypes]([KYCTypeID],[PayitServiceID],[KYCRating],[Status],[CreatedDate]) VALUES (@typeID,@serviceID,@rating,@status,getdate())")
                        cmd.Connection = cn
                        cmd.Parameters.AddWithValue("@typeID", ddlKYCType.SelectedValue)
                        ...
                        If (insert = 1) Then
                            alert("KYC successfully added for:" & ddlKYCType.Text & "!")
                        Else
    ...
    End Sub
    Protected Sub Clear()
        ddlKYCType.SelectedIndex = 0
        ddlService.SelectedIndex = 0
        txtRating.Text = String.Empty
        chkStatus.Checked = False
    End Sub
    Private Sub BindData()
        Dim strQuery As String = ("SELECT TOP 1000 psk.[ID],psk.[KYCTypeID],psk.[PayitServiceID],kt.Name KYCType, ps.ServiceCode ServiceName,psk.[KYCRating],psk.[Status],psk.CreatedDate [Date] from " & _
                                  " [payit].[dbo].[PayitServicesKYCTypes] psk LEFT JOIN [payit].[dbo].KYCTypes as kt on psk.KYCTypeID = kt.ID" & _
                                  " LEFT JOIN [payit].dbo.PayitServices as ps on psk.PayitServiceID = ps.ID order by psk.ID desc")
    ...
    Protected Sub Edit(ByVal sender As Object, ByVal e As EventArgs)
        ...
        If row.Cells(1).Text = "None" Or row.Cells(1).Text = "Select" Or row.Cells(1).Text = "&nbsp;" Or row.Cells(1).Text = "" Then
            ddlKYCTypeEdit.SelectedIndex = 0
        Else
            ddlKYCTypeEdit.SelectedValue = row.Cells(1).Text
        End If
    ...
    Protected Sub Save(ByVal sender As Object, ByVal e As EventArgs)
        ...
        If (ddlKYCTypeEdit.SelectedIndex = 0 Or ddlServiceEdit.SelectedIndex = 0 Or txtRatingEdit.Text = "") Then
            alert("Please fill all fields!")
        Else
            ...
            Using con As New SqlConnection(strConnString)
                Using cmd As New SqlCommand("UPDATE [payit].[dbo].[PayitServicesKYCTypes] SET [KYCTypeID]= '" & ddlKYCTypeEdit.SelectedValue & "', [PayitServiceID] = '" & ddlServiceEdit.SelectedValue & "', [KYCRating]='" & txtRatingEdit.Text.Trim() & "', Status = " & catStatEdit & " WHERE ID = " & txtID.Text.Trim())
    ...
    Protected Sub Delete(ByVal sender As Object, ByVal e As EventArgs)
        Using con As New SqlConnection(strConnString)
            Using cmd As New SqlCommand("DELETE FROM [payit].[dbo].[PayitServicesKYCTypes] WHERE ID = " & txtID.Text.Trim())
                Using sda As New SqlDataAdapter()
                    cmd.Connection = con
                    sda.SelectCommand = cmd
                    Using dt As New DataTable()
    ...
    Protected Sub LinkButton1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles LinkButton1.Click
        ExportToExcel.Export("PayitServiceKYCTypes.xls", Me.GridView1)
    End Sub
