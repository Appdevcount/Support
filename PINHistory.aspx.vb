...
Partial Class PINHistory
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not (Session("role") = "superadmin" Or Session("role") = "accounts") Then
            ...
            Response.Redirect("~/login.aspx?ReturnURL=PINHistory.aspx")
        End If
    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        End If
        Try
            sqlLocalTrans = "SELECT [InvoiceNumber] AS Description, [Service], [Vendor], [Info1] AS Denomination, [Amount], [UploadedPINS] AS PINS, [UploadedOn] AS Date FROM [payit].[dbo].[UploadPINSHistory] WHERE [UploadedOn] between '" & fromdate & "' and '" & todate & "'"
            ...
            If ddlServiceName.SelectedItem.Text <> "ALL" Then
                ...
                If ddlVendor.SelectedItem.Text <> "ALL" Then
                    ...
                    If tempSQL = "" Then
                        tempSQL = "[Vendor] like '" & Trim(ddlVendor.SelectedItem.Text) & "' "
                    Else
                        tempSQL = tempSQL & " and [Vendor] like '" & Trim(ddlVendor.SelectedItem.Text) & "' "
                    End If
                End If
            ...
            ElseIf ddlServiceName.SelectedItem.Text = "ALL" Then
                If tempSQL = "" Then
                    tempSQL = "[Service] like '%' "
                Else
                    tempSQL = tempSQL & " and [Service] like '%' "
                End If
            End If
          

            If (Not (String.IsNullOrEmpty(txtInvoice.Text))) Then
                tempSQL = tempSQL & " and [InvoiceNumber] like '" & txtInvoice.Text.Trim() & "' "
            End If

            If tempSQL <> "" AndAlso DenomSQL <> "" Then
                sqlLocalTrans = sqlLocalTrans & " and " & tempSQL & " and " & DenomSQL & " Group by [UploadedPINS],[UploadedOn],[Service],[Info1],[Amount],[InvoiceNumber],[Vendor] order by [Service],cast(Amount as float)"
            ElseIf tempSQL <> "" Then
                sqlLocalTrans = sqlLocalTrans & " and " & tempSQL & " Group by [UploadedPINS],[UploadedOn],[Service],[Info1],[Amount],[InvoiceNumber],[Vendor] order by [Service],cast(Amount as float)"
            ElseIf DenomSQL <> "" Then
                sqlLocalTrans = sqlLocalTrans & " and " & DenomSQL & " Group by [UploadedPINS],[UploadedOn],[Service],[Info1],[Amount],[InvoiceNumber],[Vendor] order by [Service],cast(Amount as float)"
            Else
                sqlLocalTrans = sqlLocalTrans & " Group by [UploadedPINS],[UploadedOn],[Service],[Info1],[Amount],[InvoiceNumber],[Vendor] order by [Service],Date,cast(Amount as float)"
            End If
